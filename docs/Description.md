...
- Реализовал только брание данные с терминала лицо и карту (надо еще и чтобы оператор вручную писал айди карты и лицо загружал, метод когда вручную добавляется лицо фотка загружается уже имеется) (фотка может быть 200 кб вроде, и я уверен только jpeg, ИИ конвентор сделает за 5 сек)
...
# В hikvision_add_people.py

Перемудрил с исключениями, хватило бы общего исключения HikvisionRequestFailed, остальные не нужны так то. Можно если в определенном методе словили ошибку обработали ее и поняли из за чего она в HikvisionRequestFailed. Обновить поля message: str, last_error: Optional[str] = None, last_response: Optional[str] = None, к примеру сообщение не удалось считать лицо, last_error код ошибки без изменения, и last_response так же без изменения. Там будет текст ответа reponse так как ошибки возвращает и текст ответа. К примеу 403 и там ошибка блок не могу привести пример. Ну там с терминала АПИ ошибка какая то.